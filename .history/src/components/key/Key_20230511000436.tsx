...
const Key: React.FC<Ikey> = ({ letter = "" }) => {
  ...
  if (letter === "M") {
    return (
      <>
        <div className="text-#414a5e flex h-[46px] flex-1 cursor-pointer select-none items-center justify-center rounded border-2 border-transparent bg-[#dce1ed] p-2 text-lg font-bold hover:bg-slate-400">
          {letter}
        </div>
        <div
          className="text-#414a5e flex h-[46px] flex-1 cursor-pointer select-none items-center justify-center rounded border-2 border-transparent bg-[#dce1ed] p-2 text-lg font-bold hover:bg-slate-400"
          onClick={handleEnter}
        >
          Enter
        </div>
